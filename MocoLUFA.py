...
class MocoLUFA(ControlSurface):
    __doc__ = " Script for MocoLUFA "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in MocoLUFA._active_instances:
    ...
    def __init__(self, c_instance):
        ...
        with self.component_guard():
            ...
            self._ctrl_map_sendd = []
            self._ctrl_map_sende = []
            self._ctrl_map_sendf = []
            self._ctrl_map_sendg = []
            self._ctrl_map_sendh = []
    ...
    def disconnect(self):
        ...
        self._ctrl_map_sendd = None
        self._ctrl_map_sende = None
        self._ctrl_map_sendf = None
        self._ctrl_map_sendg = None
        self._ctrl_map_sendh = None
    ...
    def _do_combine(self):
        if self not in MocoLUFA._active_instances:
            MocoLUFA._active_instances.append(self)
            MocoLUFA._combine_active_instances()


    def _do_uncombine(self):
        if ((self in MocoLUFA._active_instances) and MocoLUFA._active_instances.remove(self)):
    ...
    def _setup_mixer_control(self):
        ...
        for track in range(TRACK_NUMBER):
            ...
            strip.set_send_controls((self._ctrl_map_senda[track], self._ctrl_map_sendb[track], self._ctrl_map_sendc[track], self._ctrl_map_sendd[track], self._ctrl_map_sende[track], self._ctrl_map_sendf[track], self._ctrl_map_sendg[track], self._ctrl_map_sendh[track]))#added send d,e,f,g & h
    ...
    def _setup_device_and_transport_control(self):
        ...
        transport.set_session_record_button(self._note_map_buttons[40])
    ...
    def _load_MIDI_map(self):
        ...
        for track_id in range(TRACK_NUMBER):
            if (TRACKSENDC[track_id] == -1) :
                ...
                control.name = 'Trk_SendC_Slider_' + str(track_id)
            self._ctrl_map_sendc.append(control)
        # TRACKSENDD Sliders-- added
        for track_id in range(TRACK_NUMBER):
            if (TRACKSENDD[track_id] == -1) :
                control = None
            else:
                control = SliderElement(MIDI_CC_TYPE, TRACKSENDD_CH[track_id], TRACKSENDD[track_id])
                control.name = 'Trk_SendD_Slider_' + str(track_id)
            self._ctrl_map_sendd.append(control)
        # TRACKSENDE Sliders-- added
        for track_id in range(TRACK_NUMBER):
            if (TRACKSENDE[track_id] == -1) :
                control = None
            else:
                control = SliderElement(MIDI_CC_TYPE, TRACKSENDE_CH[track_id], TRACKSENDE[track_id])
                control.name = 'Trk_SendE_Slider_' + str(track_id)
            self._ctrl_map_sende.append(control)
        # TRACKSENDF Sliders-- added
        for track_id in range(TRACK_NUMBER):
            if (TRACKSENDF[track_id] == -1) :
                control = None
            else:
                control = SliderElement(MIDI_CC_TYPE, TRACKSENDF_CH[track_id], TRACKSENDF[track_id])
                control.name = 'Trk_SendF_Slider_' + str(track_id)
            self._ctrl_map_sendf.append(control)
        # TRACKSENDG Sliders-- added
        for track_id in range(TRACK_NUMBER):
            if (TRACKSENDG[track_id] == -1) :
                control = None
            else:
                control = SliderElement(MIDI_CC_TYPE, TRACKSENDG_CH[track_id], TRACKSENDG[track_id])
                control.name = 'Trk_SendG_Slider_' + str(track_id)
            self._ctrl_map_sendg.append(control)
        # TRACKSENDH Sliders-- added
        for track_id in range(TRACK_NUMBER):
            if (TRACKSENDH[track_id] == -1) :
                control = None
            else:
                control = SliderElement(MIDI_CC_TYPE, TRACKSENDH_CH[track_id], TRACKSENDH[track_id])
                control.name = 'Trk_SendH_Slider_' + str(track_id)
            self._ctrl_map_sendh.append(control)
        # DEVICEBANK Buttons
