...
class SpecialTransportComponent(TransportComponent):
    ...
    def __init__(self):
        ...
        song = self.song()
        self._session_record_toggle = ToggleComponent(u'session_record', song)
        return None
    ...
    def set_tempo_encoder(self, control):
        assert ((control == None) or (isinstance(control, EncoderElement) and (control.message_map_mode() is Live.MidiMap.MapMode.relative_two_compliment)))#relative_two_compliment
        if (self._tempo_encoder_control != None):
            self._tempo_encoder_control.remove_value_listener(self._tempo_encoder_value)
